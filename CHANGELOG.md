...
### Fixed
- A session waiting on a blocked-by dependency now survives a Cyrus restart. Parked sessions were held in memory only, so restarting dropped them silently: the blocker's later completion woke nothing, the issue kept the "will start automatically when it is resolved" promise forever, and even a re-prompt could not clear it. Parked sessions are now saved with the rest of the EdgeWorker state, so the blocker's completion starts the work as it always should have. ([#1494](https://github.com/cyrusagents/cyrus/pull/1494))
